    ...
    <div class="card mt-4">
        <div class="card-header">
            ...
            <div class="d-flex justify-content-between mt-3">
                <h5><a href="{{ route('users.show', ['user' => $comment->tweet->user_id]) }}">{{ $comment->tweet->user->name }}</a></h5> 
...
@elseif($activity->description == 'this comment is being liked')
    <div class="card mt-4">
        <div class="card-header">
            ...
            <div class="d-flex justify-content-between mt-3">
                <h5><a href="{{ route('users.show', ['user' => $comment->user_id]) }}">{{ $comment->user->name }}</a></h5> 
                <small>{{ $comment->created_at->diffForHumans() }}</small>
            </div>
        </div>
        <div class="card-body">
